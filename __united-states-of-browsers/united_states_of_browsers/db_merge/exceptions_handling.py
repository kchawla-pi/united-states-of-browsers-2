...
def invalid_path_in_tree(path_to_test: PathInfo) -> AnyStr:
    ...
    first_invalid_path_in_tree = [
        path_parent
        for path_parent in path_to_test.parents
        if not path_parent.exists() or not path_parent.is_dir()
    ]
...
def remove_new_empty_files(dirpath: PathInfo, existing_files: Iterable[Text]) -> None:
    ...
    files_post_connection_attempt = set(
        entry for entry in dirpath.iterdir() if entry.is_file()
    )
...
def determine_table_access_exception(
    exception_obj: Exception, calling_obj: object
) -> Exception:
